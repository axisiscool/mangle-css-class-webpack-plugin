...
const optimize = (compiler, [file, originalSource], compilation, opts, classGenerator) => {
  let classnameRegex;
  if (file.match(/.+\.css.*$/)) {
    classnameRegex = new RegExp(`\\\.(${opts.classNameCSSRegExp})`, 'g');
  } else if (file.match(/.+\.js.*$/) || file.match(/.+\.html.*$/)) {
    classnameRegex = new RegExp(`["'\`.\\\s](${opts.classNameFileRegExp})`, 'g');
  }
  if (!classnameRegex) {
    return;
  }
...
const optimizer = (opts) => {
  if (!opts.classNameCSSRegExp) throw new Error("'classNameCSSRegExp' option is required. e.g. '[c]-[a-z][a-zA-Z0-9_]*'");
  if (!opts.classNameFileRegExp) throw new Error("'classNameFileRegExp' option is required. e.g. '[c]-[a-z][a-zA-Z0-9_]*'");
